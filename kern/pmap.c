...
boot_alloc(uint32_t n)
{
	...
	//return the next free address rounded to page size
	result = ROUNDUP(nextfree, PGSIZE);

	//add n bytes of memory for the next free page address
	nextfree += n;

	//if it exceeds and becomes non kernel virtual address, Panic!
	PADDR(nextfree);

	//Round the nextfree available address to PGSIZE
...
x64_vm_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	//boot_map_region(pml4e,KERNBASE,npages * PGSIZE,0,PTE_U|PTE_P |PTE_W);
...
int
check_for_reserved_spaces(struct PageInfo *trav)
{
	if (((int *)page2kva(trav) >= (int *)EXTPHYSMEM &&
	     (int *)page2kva(trav) < (int *)boot_alloc (0)) ||
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if (check_for_reserved_spaces(&pages[i])) {
			pages[i].pp_ref++;
			continue;
		}
	...
	// Remove first page from the free list
...
{
	pml4e_t 	*pdpe_addr 	= (pml4e_t *)(pml4e + PML4(va));
	...
	if (!(*pdpe_addr & PTE_P)) {
		...
		*pdpe_addr = (pml4e_t) page2pa (new_page);
		(*pdpe_addr) |= (PTE_P | PTE_W | PTE_U);
	}

	pdpe_ptr = KADDR(PTE_ADDR(pml4e[PML4(va)]));

	addr =  pdpe_walk(pdpe_ptr, va, create);
	if(!addr && new_page) {
		*pdpe_addr = 0;
...
{
	pdpe_t		*pde_addr 	= (pdpe_t *)(pdpe+ PDPE(va));
	struct PageInfo	*new_page	= NULL;
	pde_t		*pde_ptr;
	pte_t	      	*addr;
	...
	if (!(*pde_addr & PTE_P)) {
		...
		*pde_addr = (pdpe_t) page2pa (new_page);
		(*pde_addr) |= (PTE_P | PTE_W | PTE_U);
	}
	pde_ptr = KADDR(PTE_ADDR(pdpe[PDPE(va)]));

	addr =  pgdir_walk(pde_ptr, va, create);
	if(!addr && new_page) {
		*pde_addr = 0;
		page_decref(new_page);
	}
...
{
	pde_t	*pte_addr = (pde_t *)(pgdir + PDX(va));
	struct PageInfo	*new_page	= NULL;
	pte_t *pte_ptr;
...
page_insert(pml4e_t *pml4e, struct PageInfo *pp, void *va, int perm)
{
	...
	struct PageInfo *lkupPage = NULL;
	pte_t *la = NULL;
	
	
	la = pml4e_walk(pml4e,va,1);
	if(la){
		if (*la & PTE_P)
...
page_remove(pml4e_t *pml4e, void *va)
{
	...
	lkupPage = page_lookup(pml4e,va,&pte_store);
	if(lkupPage!=NULL){
		page_decref(lkupPage);
		*pte_store = 0;
